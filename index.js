...
function removeDuplicatesInPlace(arr) {
  ...
  while (index <= arr.length - 1) {
    ...
    if (newSet.has(arr[index])) {
      // console.log(arr);
      // Якщо елемент вже є унікальним, видаляємо його з масиву
      arr.splice(index, 1)
      // console.log(arr);
      // Зменшуємо лічильник, оскільки масив став коротшим - я просто повернув цикл на початок з тим же номером лічильника
      continue;
    } else {
...
function getDifference(set1, set2) {
  ...
  for (const item of set1) {
    ...
    set2.has(item) ? '' : differenceSet.add(item)
  }
...
function getIntersection(arr1, arr2) {
  ...
  for (const item of set1) {
    ...
    set2.has(item) ? intersectionSet.add(item) : '';
